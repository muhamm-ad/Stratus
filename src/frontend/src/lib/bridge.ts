...
/** Trigger the Entra browser login. Blocks until the user completes or cancels. */
...
/** Silent credential exchange for one provider — no browser. */
export async function connectProvider(id: string): Promise<void> {
  if (!inWails) {
    await new Promise<void>(r => setTimeout(r, 800));
    return;
  }
  await App.ConnectProvider(id);
}

/**
 * List VMs for a provider. Returns real data from the cloud when running inside
 * Wails; falls back to the per-provider slice of MOCK_VMS in standalone mode.
 */
export async function listInstances(providerID: string, accountID: string): Promise<VMInstance[]> {
  if (!inWails) {
    await new Promise<void>(r => setTimeout(r, 600 + Math.random() * 600));
    return MOCK_VMS.filter(v => v.provider === (providerID as ProviderID));
  }
  const data = await App.ListInstances(providerID, accountID);
